...
from .siglip_smoe import SiglipSMoEVisionTransformer
...
class SiglipVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.image_processor = SiglipImageProcessor.from_pretrained(self.vision_tower_name, local_files_only=True)
        self.scales = args.scales
      
        if args.clip_smoe:
            self.vision_model = SiglipSMoEVisionTransformer(
                config = self.cfg_only, 
                args = args
                )
        else:
            self.vision_model = SiglipVisionTransformer(self.cfg_only)
        self.is_loaded = True
    ...
    def forward(self, images, return_id_experts = False):
        ...
        if type(images) is list:
            image_features = []
            for image in images:
                image_forward_out = self.vision_model(image.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_hidden_states=True, **kwargs)
            ...
            if self.clip_smoe:
                ...
                for i, (x, num_split) in enumerate(zip(image_pyramids, num_splits)):
                    if i == 0:
                        out_x, auxilarity_loss, vision_id_experts, infor_aux_tmp = self.vision_model( pixel_values = x,   return_id_experts = return_id_experts)
                    else:
                        out_x, auxilarity_loss, _, infor_aux_tmp = self.vision_model( pixel_values = x,   return_id_experts = return_id_experts)
                ...
                # mean some image auguments
                for i in infor_aux.keys():
                    infor_aux[i] =  torch.stack(infor_aux[i]).mean()
                return image_features, torch.stack(auxilarity_losses).mean(), vision_id_experts, infor_aux
            else:
                image_features = []
                for i, (x, num_split) in enumerate(zip(image_pyramids, num_splits)):
