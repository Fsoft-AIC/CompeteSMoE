            ...
            if self.config.mlp_smoe or self.config.clip_smoe:
                ...
                for i in aux_mlp.keys():
                    if i in aux_mlp.keys():
                        aux_mlp[i] = aux_mlp[i].sum(dim=-1).mean().detach().clone()
                        out["trainer_logs"][i + '_clip'] = aux_mlp[i]
                    if i in aux_clip.keys():
                        aux_clip[i] = aux_clip[i].sum(dim=-1).mean().detach().clone()
                    
                        out["trainer_logs"][i+ '_mlp'] = aux_clip[i]
