...
import pandas as pd
@register_moe("competesmoev29")
class CompeteSMoEv29(MoeLayer):
    ...
    def comp_v1(self, x):
        return torch.mean(F.softplus(x)**2, dim=-1)
    def comp_v2(self, x):
        return torch.mean(F.softplus(x)/0.5, dim=-1)
    def comp_v1_combine(self, x, p = 0.9):
        return torch.mean(F.softplus(x)**2, dim=-1)
        
    def compute_moe_inference(self, selected_experts, weights, results, x):
        """
        ...
        for i, expert in enumerate(self.experts):
            ...
            luna_gating[batch_idx, token_idx, i] =  self.comp_v1(out_exp)
            origin_gating[batch_idx, token_idx, i] = weights[batch_idx, token_idx, topk_idx]
        luna_gating = torch.softmax(luna_gating, dim=-1)
        p = 0.93
        luna_gating_softmax = torch.softmax(luna_gating * p + (1 - p)*origin_gating, dim=-1)
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            eloss = self.entropy_loss(gate_softmax=affinity_softmax, gate_logits=affinity_logits)
            balance_loss = self.balanceloss(selected_experts=affinity_selected_experts, gate_softmax=affinity_softmax)
            auxiliary_loss = routerloss * self.args.router_loss_coef  + balance_loss * self.args.diversity_loss_coef + eloss * self.args.e_loss_coef
