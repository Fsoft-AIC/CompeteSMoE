...
import pandas as pd
@register_moe("competesmoev33")
class CompeteSMoEv33(MoeLayer):
    ...
    def router_policy(self, x):
        ...
        """
        assert not (getattr(self.args, "is_cosine", False) == True and getattr(self.args, "is_norm_weight", False) == True), "Can not active  both  Cosine and Norm Weigh. Just use one method - Cosine or Norm Weigh to Normalization"
        if getattr(self.args, "is_cosine", False) == True and getattr(self.args, "is_norm_weight", False) == False:
            ...
            gate_logits = F.linear(x_norm, F.normalize(self.gate.weight, p=2.0, dim=-1), bias=None)
        elif getattr(self.args, "is_norm_weight", False):
            gate_logits = F.linear(x, F.normalize(self.gate.weight, p=2.0, dim=-1), bias=None)
        else:
            gate_logits = self.gate(x)

       
        if  getattr(self.args, "norm_softmax", False):
            weights, selected_experts, gate_softmax = self.topk_expert_softmax(gate_logits=gate_logits)
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            auxiliary_loss = routerloss * self.args.router_loss_coef + balance_loss * (self.args.balance_loss_coef / 2) 
            
            # Perform MoE computation using competition-selected experts
            output, _ = self.compute_moe(
                selected_experts=affinity_selected_experts,
                weights=affinity_weights,
                results=output,
                x=x,
                return_topk_outputs=False
            )
            selected_experts = affinity_selected_experts
            # print(routerloss)
            infor_aux = {
                ...
                # "level_learning": (gate_selected_experts == affinity_selected_experts).sum(),
            }
            
        else:
            # Perform MoE computation using gate-selected experts
            output, diversity_loss = self.compute_moe(
                weights=gate_weights,
                selected_experts=gate_selected_experts,
                results=output,
                x=x, 
                return_topk_outputs = self.current_steps < self.step_warm if x.requires_grad else False
            )
            if x.requires_grad: 
                auxiliary_loss, balance_loss, router_z_loss = self.combine_loss(
                    ...
                    acitve_zloss = self.current_steps >= self.step_warm
                )
                
                    
                
                
                if self.current_steps < self.step_warm:
                    auxiliary_loss += diversity_loss * self.args.diversity_loss_coef
                    
                    infor_aux = {
                    "balance_loss": balance_loss.clone().detach(),
                    }
                    infor_aux["diversity_loss"] =  diversity_loss.clone().detach()
                else:
                    infor_aux = {
                    "balance_loss": balance_loss.clone().detach(),
