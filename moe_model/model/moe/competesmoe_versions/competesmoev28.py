...
import pandas as pd
@register_moe("competesmoev28")
class CompeteSMoEv28(MoeLayer):
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            auxiliary_loss = routerloss * self.args.router_loss_coef  + diversity_loss * self.args.diversity_loss_coef + balance_loss * self.args.diversity_loss_coef + eloss * self.args.e_loss_coef
            ...
            output = self.compute_moe_inference(
