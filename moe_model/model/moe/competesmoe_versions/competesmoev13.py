...
@register_moe("competesmoev13")
class CompeteSMoEv13(MoeLayer):
    ...
    def experts_diversity_loss(self, expert_outputs, affinity_logits):
        """
        ...
        """
        affinity_softmax = self.sigmoid(affinity_logits)
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            diversity_loss = self.experts_diversity_loss(expert_outputs=expert_outputs, affinity_logits = affinity_logits)
