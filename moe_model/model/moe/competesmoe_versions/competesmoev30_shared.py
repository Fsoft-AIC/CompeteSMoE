...
import pandas as pd
import copy
@register_moe("competesmoev30S")
class CompeteSMoEv30S(MoeLayer):
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            # level_learning = (affinity_selected_experts == gate_selected_experts).sum().item()
            # self.log_metrics['level_learning'] = level_learning
        else:
