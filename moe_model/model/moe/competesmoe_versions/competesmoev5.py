...
@register_moe("competesmoev5")
class CompeteSMoEv5(MoeLayer):
    ...
    def experts_diversity_loss(self, expert_outputs):
        """
        Computes diversity loss based on Mean Squared Error (MSE) between expert outputs.

        Args:
            expert_outputs (Tensor): Tensor of shape [B, N, K, D]
                - B: Batch size
                - N: Sequence length
        ...
        Returns:
            loss (Tensor): Scalar tensor representing the mean MSE between expert outputs.
        """
        # Ensure the tensor is in float32 for precision
        ...
        loss = torch.exp(-pairwise_sq_dist).sum() / (B * N * num_pairs)
