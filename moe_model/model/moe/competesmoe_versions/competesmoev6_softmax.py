...
class CompeteSMoEv6(MoeLayer):
    def __init__(self, in_embed_dim=768, out_embed_dim=768, num_of_experts=4, num_selected=2, expert=None, args=None):
        ...
        # if self.training:
        # self.register_buffer('prob_flips', torch.zeros(8233))
    ...
    def compute_moe(self, selected_experts, weights, results, x):
        ...
        for i, expert in enumerate(self.experts):
            ...
            luna_gating[batch_idx, token_idx, i] =  torch.norm(F.softplus(out_exp), dim=-1, p = 2)
            origin_gating[batch_idx, token_idx, i] = weights[batch_idx, token_idx, topk_idx]
        p = 0.93
        luna_gating = torch.softmax(luna_gating, dim = -1) 
        luna_gating_softmax = torch.softmax(luna_gating*p + (1.0 - p)*origin_gating, dim=-1)        
        for i, expert in enumerate(self.experts):
            batch_idx, token_idx, topk_idx = infor_experts[i]
