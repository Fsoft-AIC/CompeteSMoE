...
@register_moe("competesmoev16")
class CompeteSMoEv16(MoeLayer):
    ...
    def competition_policy(self, x):
        ...
        weights, selected_experts = torch.topk(affinity_softmax.to(x.dtype), self.num_selected)
    ...
    def router_loss(self, gate_softmax, affinity_softmax):
        ...
        """
        loss = F.mse_loss(gate_softmax.to(torch.float32), affinity_softmax.to(torch.float32))
        return loss
