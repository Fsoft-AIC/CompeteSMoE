...
import pandas as pd
@register_moe("competesmoev30")
class CompeteSMoEv30(MoeLayer):
    ...
    def router_policy(self, x):
        ...
        if getattr(self.args, "norm_sigmoid", False):
            weights, selected_experts, gate_softmax = self.topk_expert_sigmoid(gate_logits=gate_logits)
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            auxiliary_loss = routerloss * self.args.router_loss_coef  + diversity_loss * (self.args.diversity_loss_coef / 2) + balance_loss * (self.args.diversity_loss_coef / 2) 
