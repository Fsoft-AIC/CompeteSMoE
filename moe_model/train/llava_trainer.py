                ...
                if len(output_logs) == 0:
                    ...
                    for k, v in ouput["trainer_logs"].items():
                        if isinstance(v, torch.Tensor):
                            if k not in output_logs.keys():
                                output_logs[k] = v / args.gradient_accumulation_steps 
                            else : output_logs[k] += v / args.gradient_accumulation_steps 
                        else:
                            output_logs[k] = v
        ...
        for module in vision_tower.modules():
            if isinstance(module, MoeLayer): 
                if hasattr(module, "total_steps"):
                    # Adaptive Layer-wise Competition Control for competesmoe
                    if  "competesmoe" in module.args.moe_name:
                        layers = module.set_total_steps(total_steps, id_layer = id_layer, prob_flips_final = layers)
                        id_layer +=1
                    else:
