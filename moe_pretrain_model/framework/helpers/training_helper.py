...
class TrainingHelper:
    ...
    def create_parser(self):
        ...
        self.arg_parser.add_argument("-save_dir", type=str, default='./save', help="Path save checkpoints")
    ...
    def start(self, restore: Optional[str]):
        ...
        if self.dist_env.is_master():
            ...
            self.summary = constructor(save_dir=os.path.join(self.args.save_dir, self.args.name) if self.args.name is not None else None,
    ...
    @master
    def log(self, origin_plotlist, step=None):
        if self.args.wandb_bug_workaround and self.use_wandb:
            filtered = {k: v for k, v in origin_plotlist.items() if not isinstance(v, visualize.plot.TextTable)}
            if len(filtered) != len(origin_plotlist) and not self.wandb_bug_found:
                print("WARNING: wandb_bug_workaround enabled. Refusing to log tables")
                self.wandb_bug_found = True
            origin_plotlist = filtered
        
        plotlist = self.fix_names(origin_plotlist)
        if plotlist:
