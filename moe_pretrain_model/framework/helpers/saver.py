...
class Saver:
    ...
    def save(self, fname: Optional[str] = None, dir: Optional[str] = None, iter: Optional[int]=None):
        ...
        log_trainer = {
            'args': state['run_invariants']['args'],
            'state': state['state'],
            "log_history": self.state_trainer,
            "Total time": self.end_time - self.start_time if self.end_time > 0 else time.time() - self.start_time
        }

        json_path = os.path.join(dir,"log_trainer.json")
        
        import json
        with open(json_path, "w") as json_file:
            json.dump(log_trainer, json_file, indent=4) 
        return fname
