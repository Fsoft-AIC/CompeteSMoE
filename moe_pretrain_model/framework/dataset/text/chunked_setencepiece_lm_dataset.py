...
class ChunkedSentencepieceLMDataset:
    ...
    def gzip_line_iterator(self, url: str):
        ...
        # for l in gzip.GzipFile(fileobj=url):
        #     yield self.parse_with_sep(l.decode("utf-8"))
        with gzip.open(url, 'rt', encoding='utf-8') as f:
            total_lines = sum(1 for line in f)  # Tính tổng số dòng trong tệp
            f.seek(0)  # Quay lại đầu tệp sau khi đếm dòng
            # print("+++++++++++++++")
    ...
    # Tối ưu hóa việc ghi file với batch processing
    def batch_write_file(self, url, out_f, batch_size=1000):
        batch = []  # List để chứa batch tokenized
        num_tokens = 0  # Đếm số lượng token đã xử lý
        batch_size = 10000000000000000000000000000000
        # Mở file đầu ra và chuẩn bị ghi
        with open(out_f, 'wb') as f:
            for l in self.line_iterator(url):
                # Token hóa dòng
                tokens = np.asarray(self.vocabulary(l), dtype=self.data_dtype)
                batch.extend(tokens)  # Thêm tokens vào batch

                # Nếu batch đầy, ghi vào file
                if len(batch) >= batch_size:
                    np.array(batch, dtype=self.data_dtype).tofile(f)  # Ghi vào file
                    num_tokens += len(batch)
                    batch = []  # Reset batch

            # Ghi bất kỳ phần còn lại nào sau khi duyệt hết dòng
            if batch:
                np.array(batch, dtype=self.data_dtype).tofile(f)
                num_tokens += len(batch)

        print(f"Đã ghi {num_tokens} tokens vào file {out_f}.")
