...
class RelativeMoeTransformerEncoderLayer(LoggingLayer, torch.nn.Module):
    def __init__(self, d_model, nhead, n_experts: int, expert_size: int, n_layers: int, dim_feedforward=2048,
        ...
        print(f"Init {args.moe_name} algorithm ...")
        self.pkm = get_moe(args.moe_name)(
        ...
        if nonorm:
            ...
            if mlp_selection:
                self.pkm.gate = torch.nn.Sequential(
                    torch.nn.LayerNorm(d_model),
                    self.pkm.gate
                )
            else:
                old_sel = self.pkm.compute_gate
                self.pkm.norm = torch.nn.LayerNorm(d_model)      
                self.pkm.compute_gate = lambda x: old_sel(self.pkm.norm(x))
    ...
    def forward(self, src: torch.Tensor, mask: Optional[AttentionMask] = None, attend_to: Optional[torch.Tensor] = None,
                pos_offset: Optional[int] = None, id_layer: Optional[int] = None) -> torch.Tensor:
        ...
        if self.preln:
            ...
            src = src2
        src3 = self.pkm(src2, id_layer = id_layer)
