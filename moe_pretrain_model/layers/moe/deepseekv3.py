...
class DeepSeekV3(MoE):
    def __init__(self, 
        ...
        #### Deepseek Setting
        self.e_score_correction_bias = torch.nn.Parameter(torch.zeros((self.n_experts)))
        self.n_group = 8
        self.topk_group = 4
        self.routed_scaling_factor = 1
        self.e_score_correction_bias_scale = 0.001
        #### ================
    ...
    def forward(self, x, return_id_experts = False, return_full = True, *args, **kwargs):
        ...
        # normlization gate network with sigmoid func
        weights, selected_experts = torch.topk(F.sigmoid(gate_logits), self.num_selected)
        ...
        if self.args.test_only:
            ...
            self.add_dist_weight(weight=F.softmax(gate_logits, dim=-1), is_all=True)
